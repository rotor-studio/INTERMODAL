...
async function loadBuses() {
  ...
  const response = await fetch("/api/emtusa/buses");
  if (!response.ok) {
...
async function loadBikeStations() {
  ...
  const response = await fetch("/api/bici/stations");
  if (!response.ok) {
    return 0;
  }
  const data = await response.json();
...
async function loadBikeParking() {
  ...
  const response = await fetch("/api/bici/parking");
  if (!response.ok) {
    return 0;
  }
  const data = await response.json();
...
async function loadCarData() {
  const response = await fetch("/api/guppy/map");
  if (!response.ok) {
    return 0;
  }
  const data = await response.json();
...
async function loadBusLines() {
  ...
  const response = await fetch("/api/emtusa/lineas-geo");
  if (!response.ok) {
    return 0;
  }
  const data = await response.json();
...
async function loadBusStops() {
  ...
  const stops = [];
  if (stopsKey === "all") {
    const response = await fetch("/api/emtusa/paradas");
    if (!response.ok) {
      return 0;
    }
    const data = await response.json();
    ...
    const responses = await Promise.all(
      selectedLineIds.map(async (lineId) => {
        const response = await fetch(`/api/emtusa/paradas?linea=${encodeURIComponent(lineId)}`);
...
async function loadSalidas(stationCode) {
  ...
  const response = await fetch(`/api/salidas?station=${stationCode}`);
  if (!response.ok) {
...
intermodalUseUserBtn?.addEventListener("click", () => {
  ...
  intermodalUserWatch = navigator.geolocation.watchPosition(
    ...
    () => {
      if (intermodalStatus) {
        intermodalStatus.innerHTML = '<div class="empty">No se pudo obtener la ubicacion.</div>';
      }
    },
